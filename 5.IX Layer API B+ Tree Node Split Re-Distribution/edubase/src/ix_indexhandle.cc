...
//split is start!!! this part is very very important //
...
//	if (result == -1 && node->GetRight() != 0) {
//		IX_BTNode* currRight = FetchNode(node->GetRight());
//		if(currRight's parent == node's parent // how to solve?? 
//			&& (currRight->GetKeysNum() < currRight->GetOrder())){
//			node->DistributeNode(currRight);
//			result = node->InsertNode(pData, rid);			
//			result = OK_RC;
//		}
//	}
    ...
    while (result == -1) {
		//check distribution, if enable => do it and break the loop.

		{
			IX_BTNode* Right, *parent;
			int pos = pathP[level-1]; // assing position of pointer in parent, that pointing the node.
			parent = path[level-1]; // assign node's parent.
			Right = FetchNode(node->GetRight()); // assign right sibling to var Right.

// for check Right node's parent...
			if(Right != NULL) {
				void * tmp_key = Right->LargestKey();
				FindLeaf(tmp_key); // for check right node's parent and original node's parent. -> change path[]
				if(parent == path[level-1]) {
					if(Right->GetKeysNum() < Right->GetOrder()) { // if there is enought space for insert.. 
						node->DistributeNode(Right);
						node->InsertNode(pData,rid);
						node->SetKey(pos, node->LargestKey());
						break;
					}
				} else {
					FindLeaf(pData); // Roll-back path[] to original node's path.
				}
			}
		}
        ...
        PF_PageHandle ph;
        PageNum p;
        RC rc = GetNewPage(p); // new page allocate
        if (rc != 0) return rc;
        rc = GetThisPage(p, ph); // 
        if (rc != 0) return rc;
        ...
        newNode = new IX_BTNode(fileHdr.attrType, fileHdr.attrLength, ph, true); // newNode
        // split into new node
        rc = node->SplitNode(newNode); // split!!!!!!!!!!................................
