...
RC IX_BTNode::DistributeNode(IX_BTNode* destNode) {
	if (keysNum > order) return -1;

	void * key;
	RID rid = *(rids + (keysNum-1)); // get node's Rightmost rid.
	key = LargestKey(); // get Rightmost Key value
	RC rc = destNode->InsertNode(key, rid); // insert entry to Right node.
	if (rc != 0) return rc;
	//move entry to right node.(shift entry and move entry from left)

	rc = DeleteNode(key, -1); // delete original node's entry.
	if (rc == -1) return rc; //
	//delete key from the original node. (last work)
...
RC IX_BTNode::SplitNode(IX_BTNode * aNewNode) {
   ...
   int movePos = (keysNum + 1) / 2; // for moving half of entries...
   ...
   for (int i = movePos; i < keysNum; i++)
   {
      ...
      if (rc != 0) return rc;
   } // start from movePos, moving to Newnode!!!
   for (int i = 0; i < moveCount; i++) {
      void * keytodel;
      GetKey(GetKeysNum() - 1, keytodel);
      RC rc = DeleteNode(keytodel, -1);
      if (rc == -1) return rc;
   } // start from end, delete entry!!! 

   aNewNode->SetRight(this->GetRight()); // newNode's right == original's right.
   this->SetRight(aNewNode->GetNodeRID().Page()); // original's right == newNode.
   aNewNode->SetLeft(this->GetNodeRID().Page()); // newNode's left == original.
